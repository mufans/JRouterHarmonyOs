...
exports.generateVarName = exports.getHashAndNode = exports.getAbilityDelegatorName = exports.addTemplate = exports.getTemplate = exports.coverageTemplate = exports.IS_FA_MODE = exports.LOCAL_TEST_SAVE_PATH = exports.COVERAGE_TYPE = void 0;
const core_1 = require("@babel/core");
const readCoverage_1 = require("./readCoverage");
const crypto_1 = require("crypto");
const constants_1 = require("./constants");
const globalTemplateVar = (0, core_1.template)(`
        var global = GLOBAL_COVERAGE_SCOPE;
`);
const globalTemplateFunc = (0, core_1.template)(`
        var global = (new Function(GLOBAL_COVERAGE_SCOPE))();
`);
const globalTemplateAlteredFunc = (0, core_1.template)(`
...
function COVERAGE_FUNCTION () {
    ...
    if (!${exports.IS_FA_MODE}) {
      ...
      if (DELEGATOR_NAME.getAbilityDelegator()) {
          global[savePath] = '/data/storage/el2/base/js_coverage.json';
          let uid = Math.floor((DELEGATOR_NAME.getAbilityDelegator().getAppContext().applicationInfo.uid)/200000);
          const bundleName = DELEGATOR_NAME.getArguments().bundleName;
        ...
        console.log('DELEGATOR_NAME.getAbilityDelegator is null when coverage data');
      }
...
function getTemplate(opts, path, T) {
    if (opts.coverageGlobalScopeFunc) {
        if (path.scope.getBinding('Function')) {
            return globalTemplateAlteredFunc({
...
function addTemplate(path, T, visitorState, countFunction) {
    ...
    let delegator = getAbilityDelegatorName(path);
    if (!delegator.isExist) {
        path.node.body.unshift(importAst.program);
    }
}
exports.addTemplate = addTemplate;
function getAbilityDelegatorName(path) {
    var _a;
    let rExp2 = /import\s(?<name>\w+)\s+(as\s+(?<alias>\w+)\s+)?from\s+["']@ohos.app.ability.abilityDelegatorRegistry["']/;
    let group = (_a = rExp2.exec(path.toString())) === null || _a === void 0 ? void 0 : _a.groups;
    if (group) {
        let delegatorName = group === null || group === void 0 ? void 0 : group.alias;
        if (!delegatorName) {
            delegatorName = group === null || group === void 0 ? void 0 : group.name;
        }
        return { 'dName': delegatorName, 'isExist': true };
    }
    return { 'dName': 'AbilityDelegatorRegistry', 'isExist': false };
}
exports.getAbilityDelegatorName = getAbilityDelegatorName;
