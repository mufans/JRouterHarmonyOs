...
const getCoverageMap = (map) => {
    ...
    for (let key of coverageKeys) {
        ...
        if (rawSourceMap.belongProject) {
            source = rawSourceMap.belongProject + source;
        }
 ...
 */
async function etsCollect(coverageDataArrayPath) {
    let etsCoverageData = {};
    try {
        const map = addDeletedMap(coverageDataArrayPath);
        let { jsCoverageData, etsTempCoverageData, keyMap, sMap, fMap, bMap } = getCoverageMap(map);
