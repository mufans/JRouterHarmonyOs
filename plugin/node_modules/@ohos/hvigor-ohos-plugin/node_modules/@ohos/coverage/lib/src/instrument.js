...
exports.instrument = exports.localTestCoveragePath = exports.unitTestInitPath = exports.ohosTestInitPath = exports.unitTest = exports.ohosTest = void 0;
const fs = require("fs");
...
exports.ohosTestInitPath = '.test/default/intermediates/ohosTest/init_coverage.json';
exports.unitTestInitPath = '.test/default/intermediates/test/init_coverage.json';
...
exports.localTestCoveragePath = '.test/default/intermediates/test/coverage_data/js_coverage.json';
...
const instrument = (instrumentParam) => {
    let { projectPath, modulePath, source, sourceMap, testFrameworkParam, id, isFaMode, overallProjectPaths, mainModulePath } = instrumentParam;
    try {
        // map文件中保存的文件路径
        ...
        let sourceMapPath = getAndCheckSourceMapPath(coverageType, modulePath);
        let sourcePath = '';
        let instrumentPath = '';
        ...
        let fileBelongProjectPath = overallProjectPaths ? getFileBelongProjectPath(mapFilePath, overallProjectPaths) : projectPath.concat(path.sep);
        if (mapFileDirPath.includes(srcOhosTest) || mapFileDirPath.includes(test)
            ...
            const [, noInstrumentSourcePath] = mapFilePath.split(fileBelongProjectPath);
        ...
        let paramMap = {
            ...
            'fileBelongProjectPath': fileBelongProjectPath,
            ...
            'overallProjectPaths': overallProjectPaths,
...
function getAndCheckSourceMapPath(coverageType, modulePath) {
    let sourceMapPath = coverageType == exports.ohosTest
...
function instrumentPart2(isFaMode, source, sourceMap, coverageType, paramMap) {
    ...
    let fileRelPath = paramMap['mapFileDirPath'].replace(paramMap['fileBelongProjectPath'], '');
    ...
    if (path.dirname(sourceFilePath) != path.dirname(instrumentFilePath)) {
        const [, sourcePath] = paramMap['mapFilePath'].split(paramMap['fileBelongProjectPath']);
        sourceMap.sources[0] = sourcePath;
        // 在模块.test下创建临时文件夹，保存插件文件和源文件
        sourceMap.belongProject = paramMap['fileBelongProjectPath'];
        ...
        let savePath = '';
        if (paramMap['mainModulePath']) {
            savePath = path.join(paramMap['mainModulePath'], exports.localTestCoveragePath).replace(/\\/g, '\\\\');
        }
...
function getFileBelongProjectPath(filePath, projectPathList) {
    for (let projectItem in projectPathList) {
        if (filePath.indexOf(projectPathList[projectItem]) !== -1) {
            return projectPathList[projectItem].concat(path.sep);
        }
    }
    return '';
}
...
function instrumentFilter(source) {
    ...
    for (let i = 0; i < length; i++) {
        ...
        if (lineData[i].includes(observeKey)) {
            lineData[i] = filterIstanbulKey + lineData[i];
        }
    }
...
function initCoverageData(source, modulePath, coverageType) {
    ...
    if (coverageType.includes(exports.ohosTest)) {
        initCoveragePath = path.join(modulePath, exports.ohosTestInitPath);
    }
    else if (coverageType.includes(exports.unitTest)) {
        initCoveragePath = path.join(modulePath, exports.unitTestInitPath);
    }
    let formatSource = '';
    // 拼接成可merge的json格式
...
function deleteMap(source) {
    const deleteReg = /function cov_[\w]{9,11}\(\) \{/;
    let isCov = false;
    let splitSign = '';
    ...
    const lineArr = source.split(splitSign);
    for (const line of lineArr) {
        if (deleteReg.test(line)) {
            isCov = true;
            break;
        }
    }
