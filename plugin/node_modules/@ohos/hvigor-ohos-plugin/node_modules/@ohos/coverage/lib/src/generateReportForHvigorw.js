...
async function generateReportForHvigorw(projectPath, reportPath, coveragePath) {
    ...
    let etsCoverageData = await (0, etsCollect_1.etsCollect)(coveragePaths);
    let data = JSON.stringify(etsCoverageData);
    (0, report_1.report)(data, reportPath, coveragePath);
}
...
function validParam(projectPath, reportPath, coveragePath) {
    ...
    if (!coveragePath) {
        ...
        ensureInitCoverageJson(coveragePath);
    }
    return true;
}
function ensureInitCoverageJson(coveragePath) {
    let coveragePaths = coveragePath.split('#');
    for (const item of coveragePaths) {
        if (item.endsWith('init_coverage.json')) {
            fs.ensureFileSync(item);
            break;
        }
    }
}
